...
function App() {
  ...
  const bcnSchools = allSchool.filter(
    (data) => data.province === "Baja California"
  );
  const bcsSchools = allSchool.filter(
    (data) => data.province === "Baja California Sur"
  );
  const camSchools = allSchool.filter((data) => data.province === "Campeche");

  const chpSchools = allSchool.filter((data) => data.province === "Chiapas");
  const chhSchools = allSchool.filter((data) => data.province === "Chihuahua");
  const coaSchools = allSchool.filter((data) => data.province === "Coahuila");
  const colSchools = allSchool.filter((data) => data.province === "Colima");
  const difSchools = allSchool.filter(
  ...
  const jalSchools = allSchool.filter((data) => data.province === "Jalisco");
  const mexSchools = allSchool.filter(
    (data) => data.province === "Estado de México"
  );
  const micSchools = allSchool.filter((data) => data.province === "Michoacán");

  const morSchools = allSchool.filter((data) => data.province === "Morelos");
  const naySchools = allSchool.filter((data) => data.province === "Nayarit");
  const nelSchools = allSchool.filter((data) => data.province === "Nuevo León");
  const oaxSchools = allSchool.filter((data) => data.province === "Oaxaca");
  const pueSchools = allSchool.filter((data) => data.province === "Puebla");
  const queSchools = allSchool.filter((data) => data.province === "Querétaro");
  const rooSchools = allSchool.filter(
    (data) => data.province === "Quintana Roo"
  );
  const slpSchools = allSchool.filter(
    (data) => data.province === "San Luis Potosí"
  );
  const sinSchools = allSchool.filter((data) => data.province === "Sinaloa");
  const sonSchools = allSchool.filter((data) => data.province === "Sonora");
  const tabSchools = allSchool.filter((data) => data.province === "Tabasco");
  const tamSchools = allSchool.filter((data) => data.province === "Tamaulipas");
  const tlaSchools = allSchool.filter((data) => data.province === "Tlaxcala");
  const verSchools = allSchool.filter((data) => data.province === "Veracruz");
  const yucSchools = allSchool.filter((data) => data.province === "Yucatán");
  const zacSchools = allSchool.filter((data) => data.province === "Zacatecas");

  const hombres = durSchools
    ? durSchools
        .flatMap((data) =>
          data.alumnos.map((row) => Number(row.cantidadAlumnos))
        )
        .reduce((total, cantidad) => total + cantidad, 0)
    : 0;

  const mujeres = durSchools
    ? durSchools
        .flatMap((data) =>
          data.alumnas.map((row) => Number(row.cantidadAlumnas))
        )
        .reduce((total, cantidad) => total + cantidad, 0)
    : 0;
  ...
  // Calcula el porcentaje de alumnos y alumnas
  const porcentajeHombres =
    totalAlumnos > 0 ? (hombres / totalAlumnos) * 100 : 0;
  const porcentajeMujeres =
    totalAlumnos > 0 ? (mujeres / totalAlumnos) * 100 : 0;

  const teacher = durSchools
    ? durSchools
        .flatMap((data) =>
          data.profesores.map((row) => Number(row.textProfeactuales.length))
        )
        .reduce((total, cantidad) => total + cantidad, 0)
    : 0;

  const doctorTeacher = durSchools
    ? durSchools
        .flatMap((data) =>
          data.profesoresConDoctorados.map((row) =>
            Number(row.textProfeDoctorado.length)
          )
        )
        .reduce((total, cantidad) => total + cantidad, 0)
    : 0;

  const masterTeacher = durSchools
    ? durSchools
        .flatMap((data) =>
          data.profesoresMaestrias.map((row) =>
            Number(row.textProfeMaestria.length)
          )
        )
        .reduce((total, cantidad) => total + cantidad, 0)
    : 0;
  ...
  const regions = {
    ...
    dur: {
      ...
      porcentaje_alumno: porcentajeHombres,
      porcentaje_alumna: porcentajeMujeres,
      current_professor: teacher,
      doctor_professor: doctorTeacher,
  ...
  const handleMouseEnter = (e, regionId) => {
    setHoveredRegion(regions[regionId]);
  ...
  const handleOpen = (regionId) => {
    setOpen(true);
    setModalRegion(regions[regionId]);
  };
    ...
    <div className="about-page">
      ...
      <main class="main">
            ...
            <div class="container">
              ...
              >
                <div class="col-lg-6 text-center">
                  <h2>Subsistema de Escuelas Normales Superiores (SENS)</h2>
                  <p>
                    Escuelas cuyo objetivo es la formación de maestros de
                    educación secundaria y pertenecen al subsistema de enseñanza
                    normal en el país.
                  </p>
                </div>
              </div>
          ...
          >
            ...
            >
              <span
                ...
                aria-hidden="true"
              >
                <IoArrowForward className="icon-color" />
              </span>
            </a>
          </div>
        </section>
        <div class="container section-title" data-aos="fade-up">
          ...
          <p>
            Bienvenidos al portal dedicado a las Escuelas Normales Superiores en
            México. Aquí, exploraremos la historia y la relevancia de estas
            instituciones en la educación nacional. Desde su establecimiento
            hasta su significativo papel en la formación docente, proporcionamos
            una perspectiva detallada y recursos educativos pertinentes.
          </p>
        </div>
        <section id="constructions" class="constructions section">
          <div class={viewInfo ? "container data-map" : "container"}>
            <div class="row gy-4  map-container">
              <svg
              ...
              >
                <path
                  ...
                  id="MX-AGU"
                  aria-describedby="Aguascalientes"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/aguascalientes"
                  onMouseEnter={(e) => handleMouseEnter(e, "agu")}
                  onClick={(e) => handleOpen(e, "agu")}
                  onMouseLeave={handleMouseLeave}
                ></path>
                <path
                  ...
                  id="MX-BCN"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/baja-california"
                  onMouseEnter={(e) => handleMouseEnter(e, "bcn")}
                  onClick={() => handleOpen("bcn")}
                  onMouseLeave={handleMouseLeave}
                ></path>
                <path
                  ...
                  id="MX-BCS"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/baja-california-sur"
                  onMouseEnter={(e) => handleMouseEnter(e, "bcs")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("bcs")}
                ></path>
                ...
                <path
                  ...
                  id="MX-CAM"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/campeche"
                  onMouseEnter={(e) => handleMouseEnter(e, "cam")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("cam")}
                ></path>
                ...
                <path
                  ...
                  id="MX-CHP"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/chiapas"
                  onMouseEnter={(e) => handleMouseEnter(e, "chp")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("chp")}
                ></path>
                <path
                  ...
                  id="MX-CHH"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/chihuahua"
                  onMouseEnter={(e) => handleMouseEnter(e, "chh")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("chh")}
                ></path>
                <path
                  ...
                  id="MX-COA"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/coahuila"
                  onMouseEnter={(e) => handleMouseEnter(e, "coa")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("coa")}
                ></path>
                ...
                <path
                  ...
                  id="MX-COL"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/colima"
                ...
                <path
                  ...
                  id="MX-DIF"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/ciudad-de-mexico"
                  onMouseEnter={(e) => handleMouseEnter(e, "dif")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("dif")}
                ></path>
                <path
                  ...
                  id="MX-DUR"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/durango"
                  onMouseEnter={(e) => handleMouseEnter(e, "dur")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("dur")}
                ></path>
                <path
                  ...
                  id="MX-GUA"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/guanajuato"
                  onMouseEnter={(e) => handleMouseEnter(e, "gua")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("gua")}
                ></path>
                <path
                  ...
                  id="MX-GRO"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/guerrero"
                  onMouseEnter={(e) => handleMouseEnter(e, "gro")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("gro")}
                ></path>
                <path
                  ...
                  id="MX-HID"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/hidalgo"
                  onMouseEnter={(e) => handleMouseEnter(e, "hid")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("hid")}
                ></path>
                <path
                  ...
                  id="MX-JAL"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/jalisco"
                  onMouseEnter={(e) => handleMouseEnter(e, "jal")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("jal")}
                ></path>
                <path
                  ...
                  id="MX-MEX"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/estado-de-mexico"
                  onMouseEnter={(e) => handleMouseEnter(e, "mex")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("mex")}
                ></path>
                <path
                  ...
                  id="MX-MIC"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/michoacan"
                  onMouseEnter={(e) => handleMouseEnter(e, "mic")}
                  onMouseLeave={handleMouseLeave}
                ...
                <path
                  ...
                  id="MX-MOR"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/morelos"
                  onMouseEnter={(e) => handleMouseEnter(e, "mor")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("mor")}
                ></path>
                <path
                  ...
                  id="MX-NAY"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/nayarit"
                  onMouseEnter={(e) => handleMouseEnter(e, "nay")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("nay")}
                ></path>
                <path
                  ...
                  id="MX-NEL"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/nuevo-leon"
                  onMouseEnter={(e) => handleMouseEnter(e, "nel")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("nel")}
                ></path>
                <path
                  ...
                  id="MX-OAX"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/oaxaca"
                  onMouseEnter={(e) => handleMouseEnter(e, "oax")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("oax")}
                ></path>
                <path
                  ...
                  id="MX-PUE"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/puebla"
                  onMouseEnter={(e) => handleMouseEnter(e, "pue")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("pue")}
                ></path>
                <path
                  ...
                  id="MX-QUE"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/queretaro"
                  onMouseEnter={(e) => handleMouseEnter(e, "que")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("que")}
                ></path>
                <path
                  ...
                  id="MX-ROO"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/quintana-roo"
                  onMouseEnter={(e) => handleMouseEnter(e, "roo")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("roo")}
                ></path>
                <path
                  ...
                  id="MX-SLP"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/san-luis-potosi"
                  onMouseEnter={(e) => handleMouseEnter(e, "slp")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("slp")}
                ></path>
                <path
                  ...
                  id="MX-SIN"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/sinaloa"
                  onMouseEnter={(e) => handleMouseEnter(e, "sin")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("sin")}
                ></path>
                <path
                  ...
                  id="MX-SON"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/sonora"
                  onMouseEnter={(e) => handleMouseEnter(e, "son")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("son")}
                ></path>
                <path
                  ...
                  id="MX-TAB"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/tabasco"
                ...
                <path
                  ...
                  id="MX-TAM"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/tamaulipas"
                  onMouseEnter={(e) => handleMouseEnter(e, "tam")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("tam")}
                ></path>
                <path
                  ...
                  id="MX-TLA"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/tlaxcala"
                  onMouseEnter={(e) => handleMouseEnter(e, "tla")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("tla")}
                ></path>
                <path
                  ...
                  id="MX-VER"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/veracruz"
                  onMouseEnter={(e) => handleMouseEnter(e, "ver")}
                  onMouseLeave={handleMouseLeave}
                ...
                <path
                  ...
                  id="MX-YUC"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/yucatan"
                  onMouseEnter={(e) => handleMouseEnter(e, "yuc")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("yuc")}
                ></path>
                <path
                  ...
                  id="MX-ZAC"
                  class="mapsvg-region"
                  data-url="https://normalessuperiores.org.mx/entidad/zacatecas"
                  onMouseEnter={(e) => handleMouseEnter(e, "zac")}
                  onMouseLeave={handleMouseLeave}
                  onClick={() => handleOpen("zac")}
                ></path>
              </svg>
              {hoveredRegion && (
                <Box
                  sx={{
                    ...
                    padding: "8px",
                    boxShadow: 2,
                    pointerEvents: "none", // Para que el tooltip no interfiera con los eventos del mouse
                    zIndex: 9999,
                  }}
                >
                  ...
                  {hoveredRegion &&
                    hoveredRegion.name_school &&
                    Array.isArray(hoveredRegion.name_school) && (
                      <ul>
              ...
              >
                <Fade in={open}>
                  <Box sx={style}>
                    <h4>{modalRegion && modalRegion.name} </h4>
                    <p>{modalRegion && modalRegion.description}</p>
                    {modalRegion &&
                  ...
                  <div className="container-students-doctors">
                    <div className="data-students">
                      <div className="total-students">
                        Total: {modalRegion.total_alumnos}
                      </div>
                      <div className="porcentaje-students">
                        <div className="students-chart">
                          <div className="circle">
                            ...
                            <div
                              ...
                              style={{
                                background: `conic-gradient(#ff99cc 0 ${modalRegion.porcentaje_alumna}%, #99ccff ${modalRegion.porcentaje_alumna}% 100%)`,
                              }}
                            ></div>
                          </div>
                        </div>
                      ...
                      <div>
                        <div>
                          Alumnas:{" "}
                          {Number(modalRegion.porcentaje_alumna).toFixed(2)}%
                        </div>
                        <div>
                          Alumnos:{" "}
                          {Number(modalRegion.porcentaje_alumno).toFixed(2)}%
                        </div>
                      </div>
                    </div>
                    <div>
                      <div>
                        ...
                        <div>
                          <div className="teacher-current">
                            <div>
                              Actuales: {modalRegion.current_professor}
                              <input
                                type="range"
                                value={modalRegion.current_professor}
                                name=""
                                id=""
                              />
                            </div>
                          ...
                          <div className="teacher-current">
                            <div>
                              Doctorados: {modalRegion.doctor_professor}
                              <input
                                type="range"
                                value={modalRegion.doctor_professor}
                                name=""
                                id=""
                              />
                            </div>
                          ...
                          <div className="teacher-current">
                            <div>
                              Maestria: {modalRegion.master_professor}
                              <input
                                type="range"
                                value={modalRegion.master_professor}
                                name=""
                                id=""
                              />
                            </div>
                    ...
                    <div>
                      <div>
                        ...
                        <div>
                          <div className="teacher-current">
                            <div>
                              Candidatos: {modalRegion.doctor_candidato}
                              <input
                                type="range"
                                value={modalRegion.doctor_candidato}
                                name=""
                                id=""
                              />
                            </div>
                          ...
                          <div className="teacher-current">
                            <div>
                              ...
                              <input
                                type="range"
                                value={modalRegion.doctor_jubilado}
                                name=""
                                id=""
                              />
                            </div>
        ...
        <section
          id="services"
          class="services section mobile-province light-background"
        >
          <div class="container" data-aos="fade-up">
            <div class="row no-gutters">
              <div class="col-12  ">
                <ul>
                  <li value="Aguascalientes">
                    Aguascalientes{" "}
                    <Link to="/Aguascalientes">Ver escuelas</Link>
                  </li>
                  <li value="Baja California">
                    Baja California{" "}
                    <Link to="/Baja-California">Ver escuelas</Link>
                  </li>
                  <li value="Baja California Sur">
                    Baja California Sur Baja California{" "}
                    <Link to="/Baja-California-Sur">Ver escuelas</Link>
                  </li>
                  <li value="Campeche">
                    Campeche <Link to="/Campeche">Ver escuelas</Link>
                  </li>
                  <li value="Chiapas">
                    Chiapas <Link to="/Chiapas">Ver escuelas</Link>
                  </li>
                  <li value="Chihuahua">
                    Chihuahua <Link to="/Chihuahua">Ver escuelas</Link>
                  </li>
                  <li value="Coahuila de Zaragoza">
                    Coahuila de Zaragoza{" "}
                    <Link to="/Coahuila">Ver escuelas</Link>
                  </li>
                  <li value="Colima">
                    Colima <Link to="/Colima">Ver escuelas</Link>
                  </li>
                  <li value="Durango">
                    Durango <Link to="/Durango">Ver escuelas</Link>
                  </li>
                  <li value="Guanajuato">
                    Guanajuato{" "}
                    <Link to="/CampGuanajuatoeche">Ver escuelas</Link>
                  </li>
                  <li value="Guerrero">
                    Guerrero <Link to="/Guerrero">Ver escuelas</Link>
                  </li>
                  <li value="Hidalgo">
                    Hidalgo <Link to="/Hidalgo">Ver escuelas</Link>
                  </li>
                  <li value="Jalisco">
                    Jalisco <Link to="/Jalisco">Ver escuelas</Link>
                  </li>
                  <li value="Estado de México">
                    Estado de México{" "}
                    <Link to="/Estado-de-México">Ver escuelas</Link>
                  </li>
                  <li value="Michoacán de Ocampo">
                    Michoacán de Ocampo{" "}
                    <Link to="/Michoacán">Ver escuelas</Link>
                  </li>
                  <li value="Morelos">
                    Morelos <Link to="/Morelos">Ver escuelas</Link>
                  </li>
                  <li value="Nayarit">
                    Nayarit <Link to="/Nayarit">Ver escuelas</Link>
                  </li>
                  <li value="Nuevo León">
                    Nuevo León <Link to="/Nuevo-León">Ver escuelas</Link>
                  </li>
                  <li value="Oaxaca">
                    Oaxaca <Link to="/Oaxaca">Ver escuelas</Link>
                  </li>
                  <li value="Puebla">
                    Puebla <Link to="/Puebla">Ver escuelas</Link>
                  </li>
                  <li value="Querétaro">
                    Querétaro <Link to="/Querétaro">Ver escuelas</Link>
                  </li>
                  <li value="Quintana-Roo">
                    Quintana Roo <Link to="/Quintana-Roo">Ver escuelas</Link>
                  </li>
                  <li value="San Luis Potosí">
                    San Luis Potosí{" "}
                    <Link to="/San-Luis-Potosí">Ver escuelas</Link>
                  </li>
                  <li value="Sinaloa">
                    Sinaloa <Link to="/Sinaloa">Ver escuelas</Link>
                  </li>
                  <li value="Sonora">
                    Sonora <Link to="/Sonora">Ver escuelas</Link>
                  </li>
                  <li value="Tabasco">
                    Tabasco <Link to="/Tabasco">Ver escuelas</Link>
                  </li>
                  <li value="Tamaulipas">
                    Tamaulipas <Link to="/Tamaulipas">Ver escuelas</Link>
                  </li>
                  <li value="Tlaxcala">
                    Tlaxcala <Link to="/Tlaxcala">Ver escuelas</Link>
                  </li>
                  <li value="Veracruz de Ignacio de la Llave">
                    Veracruz de Ignacio de la Llave{" "}
                    <Link to="/Veracruz">Ver escuelas</Link>
                  </li>
                  <li value="Yucatán">
                    Yucatán <Link to="/Yucatán">Ver escuelas</Link>
                  </li>
                  <li value="Zacatecas">
                    Zacatecas <Link to="/Zacatecas">Ver escuelas</Link>
                  </li>
                </ul>
              </div>
            </div>
